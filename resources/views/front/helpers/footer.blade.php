<footer class="footer section">
    <div class="container">
        <div class="row">
            <div class="col-lg-2 col-md-6 col-sm-6">
                <div class="widget">
                    <h4 class="text-capitalize mb-4">Quick Links</h4>

                    <ul class="list-unstyled footer-menu lh-35">
                        <li><a href="{{route('front.index')}}">Home</a></li>
                        <li><a href="{{route('front.about')}}">About</a></li>
                        <li><a href="{{route('front.treatment')}}">TREATMENT</a></li>
                        <li><a href="{{route('front.tour')}}">TOUR</a></li>
                        <li><a href="{{route('front.testimonials')}}">TESTIMONIALS</a></li>
                        <li><a href="{{route('front.admissions')}}">ADMISSIONS</a></li>
                        <li><a href="{{route('front.blog')}}">BLOG</a></li>
                        <li><a href="{{route('front.contact')}}">Contact</a></li>
                    </ul>
                </div>
            </div>

            <div class="col-lg-3 ml-auto col-sm-6">
                <div class="widget">
                    <div class="logo mb-4">
                        <h3>Mega<span>kit.</span></h3>
                    </div>
                    <h6><a href="mailto:{{$contact->email}}" >{{$contact->email}}</a></h6>
                    <a href="tel:{{$contact->phone}}"><span class="text-color h4">{{$contact->phone}}</span></a>
                </div>
            </div>
        </div>

        <div class="footer-btm pt-4">
            <div class="row">
                <div class="col-lg-4 col-md-12 col-sm-12">
                    <div class="copyright">
                        &copy; Copyright 2022 Recovery Health
                    </div>
                </div>

                <div class="col-lg-4 col-md-12 col-sm-12">
{{--                    <div class="copyright">--}}
{{--                        Distributed by  <a href="https://themewagon.com/" target="_blank">Themewagon</a>--}}
{{--                    </div>--}}
                </div>
                <div class="col-lg-4 col-md-12 col-sm-12 text-left text-lg-left">
                    <ul class="list-inline footer-socials">
                        <li class="list-inline-item"><a href="https://www.facebook.com/themefisher"><i class="ti-facebook mr-2"></i>Facebook</a></li>
                        <li class="list-inline-item"><a href="https://twitter.com/themefisher"><i class="ti-twitter mr-2"></i>Twitter</a></li>
                        <li class="list-inline-item"><a href="https://www.pinterest.com/themefisher/"><i class="ti-linkedin mr-2 "></i>Linkedin</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</footer>
